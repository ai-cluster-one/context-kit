...
class UpdateTests(unittest.TestCase):
    ...
    def test_previous_0_1_1_release_updates_to_current_patch_release(self) -> None:
        ...
        self.assertEqual(current["version"], "0.1.3")
        release = self.remote / self.ref
        ...
        self.assertEqual(check_report["available"]["version"], "0.1.3")
        ...
        self.assertEqual(apply_report["current"]["version"], "0.1.3")
    ...
    def test_previous_0_1_2_release_updates_to_0_1_3(self) -> None:
        self.write_release(version="0.1.2")
        installed = self.run_cli("update", "--apply", "--yes", "--json")
        self.assertEqual(installed.returncode, 0, installed.stderr)
        self.assertEqual(json.loads((self.install_home / "release.json").read_text())["version"], "0.1.2")
        ...
        check_report = json.loads(checked.stdout)
        self.assertEqual(check_report["status"], "update_available")
        self.assertEqual(check_report["current"]["version"], "0.1.2")
        self.assertEqual(check_report["available"]["version"], "0.1.3")
